...
set the state. Third, and most consequentially, \emph{identifiability and predictive uncertainty are
distinct, non-redundant properties}: neither determines the other. This claim is tested
within a single pre-registered stochastic hypothesis class, where all four combinations of
generator identifiability and one-step predictive uncertainty are populated under one
fixed pair of definitions. The deterministic systems separately illustrate that generator
recovery need not provide computational shortcuts or forward determinacy.
...
that failed the exploratory criterion, reproduced on an unrelated family. A longer horizon does not help, and we
measured rather than assumed this. Sweeping $T = 1,\dots,8$ under the passive protocol,
the partition refines to $303{,}229$ classes at $T=6$ and then stops: $303{,}229$ again at
$T=7$ and at $T=8$, with $H(G\mid O_T) = 3.245$ bits throughout and no generator uniquely
identified at any horizon. The observed stabilisation point is $T^\star = 6$, which is the
horizon Experiment~20 fixed in advance. Stability through $T=8$ is not a proof of
stabilisation for all $T$---that requires the finite-rank argument implicit
in~\cite{ito1992}---but it removes the possibility that the empty passive row is an
artefact of looking for too short a time. Second, the upper-right quadrant is
...
mechanism. Third, C5 at $82.0\%$ says quadrant membership is largely not a property of the system.
The migration is worth stating exactly, because its shape is constrained by the design:
$1{,}230{,}684$ generators become identifiable, \emph{zero} lose identifiability, and
\emph{zero} move in the predictive coordinate. That $p_1$ does not move is by
construction---we evaluate it under the uniform operating condition in both protocols so
that only the identification channel varies---so migration here is necessarily
one-dimensional and one-directional. The claim is therefore narrower than ``quadrant
membership is regime-dependent'' in a two-dimensional sense: it is the identifiability
coordinate that is regime-dependent, and the predictive coordinate was held fixed and
could not have moved. The pipeline is
\[
  (S, \mathcal{A}, \Sigma, O)
  \;\longrightarrow\;
  \big(i_O(g),\, p_k(g)\big)
...
The two coordinates in fact differ in domain of definition, not merely in value. $i_O(g)$
is a finite integer for every member of a finite hypothesis class, by construction.
$C_\mu$ is a Shannon entropy over causal states, and for a hidden Markov process those
states are, as Jurgens and Crutchfield show, generically infinite in
number~\cite{jurgens2021}: finite-state generators produce processes requiring an
uncountable infinity of predictive features, for which the finite entropy must be replaced
by a statistical complexity \emph{dimension}. We observe this directly. Constructing the
mixed-state presentation for the $3375$-generator exploratory family of
\S\ref{sec:stochastic}, only $40.8\%$ terminate within a $400$-state budget; of $60$
excluded generators re-run at a $20{,}000$-state budget, none terminated. The excluded set
is not otherwise unusual---its mean $i_O$ is $2.217$ bits against $2.325$ for the
retained---so this is a property of $C_\mu$'s constructibility, not of a strange corner of
the class. Computational mechanics has the machinery for that case; our point is that it
is needed there and not for $i_O$.
...
horizon $T=6$, not of the full process law, so every preimage reported there is formally an
upper bound on the preimage available with unlimited passive data. The horizon sweep
narrows that gap considerably---the partition is unchanged at $T=7$ and $T=8$---but
stability over a finite window is evidence, not proof, and the empty passive row remains a
statement about the declared observation map rather than about structural identifiability
in the sense of~\cite{ito1992}.
...
ill-posed: minima are non-unique, unstable, and lack a closed form. In the cellular-automaton
system, rule recovery is trivial but does not provide a useful local landscape for
anticipating behavioural complexity.
...
All figures are produced by nine self-contained Python scripts requiring only the
...
\texttt{mgs\_preregistered.py} (the frozen confirmatory run),
\texttt{mgs\_epsilon.py} (the mixed-state construction behind the $C_\mu$ comparison of
\S\ref{sec:related}) and \texttt{mgs\_lattice.py} (the horizon sweep). The last two of
these additionally require NumPy; all but those run in a few minutes on one core.
...
fixed seed. Export keys are assigned by hand rather than derived from the claim text, and
a duplicate key is a hard error: a derived key changes silently when prose is reworded,
and two claims differing only in punctuation collide, both of which occurred. The current run is $45$ of $45$ claims verified, recorded in
...
\bibitem{jurgens2021} A. M. Jurgens and J. P. Crutchfield. Divergent predictive states:
the statistical complexity dimension of stationary, ergodic hidden Markov processes.
arXiv:2102.10487, 2021.
