...
$K_T \subseteq \mathbf{1}^\perp$: every semantic-kernel direction has zero coordinate sum.
...
$T_{\mathrm{sem}} = \min\{T : K_T = K_\infty\}$,
\[
  T_{\mathrm{sem}} \;\le\; n - k + 1,
  \qquad\text{so}\qquad
  K_\infty = K_{\,n-k+1}.
\]
\end{proposition}
...
$T_{\mathrm{sem}} \le 1$, recovering Corollary~\ref{cor:inj}. For the four-state balanced
...
$T_{\mathrm{sem}}$ exceeds $n-k+1$ and no plateau is later broken; and for each $k$ the
maximum is reached---$T_{\mathrm{sem}} = 3$ occurs $94$ times at $k=2$, $T_{\mathrm{sem}} = 2$
occurs $265$ times at $k=3$, and every case at $k=4$ has $T_{\mathrm{sem}} = 1$. So the
...
rank, no $T_{\mathrm{sem}}$ exceeds the bound, and it is attained for
...
Thus $T_{\mathrm{sem}} = 1$ whenever $\operatorname{rank} C = 1$, and the bound $n$ is
...
rank gives $T_{\mathrm{sem}} = 1$ without any deterministic state label. We claim Corollary~\ref{cor:stoch} as a transparent rank-sensitive
...
This is why the semantic kernel is the right object rather than a refinement of the state
