...
The explanation we favour, and flag as post-hoc, is that coverage was the wrong statistic.
A singleton block carries no internal ambiguity---the observation already identifies that
state---so a reset spent there buys nothing, and what matters is how much an intervention
probes blocks that are internally ambiguous. Crossing correlates with that only when the
blocks are equal in size. The corrected statement would be about block sizes rather than
block counts, and we do not yet have it in testable form.
...
partitions; the replacement hypothesis about internally ambiguous blocks is post-hoc and
untested. Submodularity is a theorem here (Proposition~\ref{prop:submod}) and holds whenever exact
