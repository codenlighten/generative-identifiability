...
Two dissociations follow, and together they say where each structure lives. Changing the
observation map moves the rank values $r(A)$ but leaves the basis untouched; introducing a
dependency among generators leaves the qualitative rank profile recognisable but collapses
the basis. The experiment family has a small informational basis precisely when the
generator family is constrained.
...
Table~\ref{tab:lattice} is a property of that construction. The compression results of
Table~\ref{tab:structure} use one dependency---a single tied row---and we have not
characterised which dependencies produce which bases. The crossing conjecture is refuted in general and survives only for equal-block
