...
The converse count is $0$. Across every class, partition and subset, no $j$ lay in
$\operatorname{cl}_Z(A)$ without lying in $\operatorname{cl}_G(A)$: the observation map
never manufactured a dependency, only destroyed them. We report
$\operatorname{cl}_Z(A) \subseteq \operatorname{cl}_G(A)$ as an empirical regularity over
$6 \times 14 \times 16$ cases rather than a theorem, since we have no proof and $Z_j$'s
dependence on the full generator does not obviously force it.
...
Table~\ref{tab:lattice} is a property of that construction. We do not know which dependency types make the basis
observation-dependent: Table~\ref{tab:dependency} shows equality and adjacent ties are
invariant while permutation and two-row derived constraints are not, and four classes are
too few to see the pattern. The regularity
$\operatorname{cl}_Z \subseteq \operatorname{cl}_G$ is unproved. The crossing conjecture is refuted in general and survives only for equal-block
