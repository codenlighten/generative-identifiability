...
ever a proxy for low redundancy, and Experiment~24 locates where the proxy fails: a reset
into a singleton observational block is largely redundant, because the observation already
identifies that state, while two resets inside a large collapsed block can expose
distinctions passive observation suppresses. That is why the effect reverses on
$\{0\} \mid \{1,2,3\}$ and holds on balanced partitions.
